...
if __name__ == "__main__":
   ...
   parser.add_argument("--test", action="store_true", help="whether to run a test after training")
   ...
   # Initialize the environment using FlattenObservation wrapper
   env = FlattenObservation(FishEnv(name=args.name, render_mode=args.render_mode))
   ...
   # Load trained actor network if specified
   if args.output is not None:
       actor_net = torch.load(args.output, map_location=device)
       actor_net.eval()


   # Test the trained model if --test flag is provided
   if args.test:
       print("Testing the trained model...")
       show_episode(args.name, args.render_mode)
