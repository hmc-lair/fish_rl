...
class FishEnv(gym.Env):
    ...
    def __init__(self, max_v=5, max_omega=2*np.pi, render_mode=None, n_fish=0, bounds=np.array([[0, 0], [10, 10]]), seed=None):
    ...
    def step(self, action):
        ...
        reward = -np.sqrt(np.sum(np.square(self._agent[:2])))
